...
export default function TechnologyPage() {
    ...
    async function load() {
      try {
        const [articlesRes, subRes] = await Promise.all([
          fetch('/api/articles?category=technology').then(r => r.json()),
          fetch('/api/articles/subcategories?category=technology').then(r => r.json()),
        ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <HeroSection category="Technology" articles={articles} />
...
function SubcategorySection({ subcategory }: { subcategory: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles?category=technology&subcategory=${encodeURIComponent(subcategory)}&limit=6`)
      .then(r => r.json())
