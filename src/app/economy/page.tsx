...
export default function EconomyPage() {
    ...
    async function load() {
      try {
        const [articlesRes, subRes] = await Promise.all([
          fetch('/api/articles?category=economy').then(r => r.json()),
          fetch('/api/articles/subcategories?category=economy').then(r => r.json()),
        ]);
        ...
        console.error('Error fetching economy page data:', e);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <HeroSection category="Economy" articles={articles} />
...
function SubcategorySection({ subcategory }: { subcategory: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles?category=economy&subcategory=${encodeURIComponent(subcategory)}&limit=6`)
      .then(r => r.json())
      .then(data => {
