...
export default function BusinessPage() {
    ...
    async function load() {
      try {
        const [articlesRes, subRes] = await Promise.all([
          fetch('/api/articles?category=business').then(r => r.json()),
          fetch('/api/articles/subcategories?category=business').then(r => r.json()),
        ]);
        ...
        console.error('Error fetching business page data:', e);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <HeroSection category="Business" articles={articles} />
...
function SubcategorySection({ subcategory }: { subcategory: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles?category=business&subcategory=${encodeURIComponent(subcategory)}&limit=6`)
      .then(r => r.json())
      .then(data => {
        if (data.success) setItems(data.articles);
      })
      .catch(() => {});
  }, [subcategory]);
