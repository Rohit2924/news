...
export default function SportsPage() {
    ...
    async function load() {
      try {
        const [articlesRes, subRes] = await Promise.all([
          fetch('/api/articles?category=sports').then(r => r.json()),
          fetch('/api/articles/subcategories?category=sports').then(r => r.json()),
        ]);
        ...
        console.error('Error fetching sports page data:', e);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <HeroSection category="Sports" articles={articles} />
...
function SubcategorySection({ subcategory }: { subcategory: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles?category=sports&subcategory=${encodeURIComponent(subcategory)}&limit=6`)
      .then(r => r.json())
      .then(data => {
