...
export default function HealthPage() {
    ...
    async function load() {
      try {
        const [articlesRes, subRes] = await Promise.all([
          fetch('/api/articles?category=health', { cache: 'no-store' }).then(r => r.json()),
          fetch('/api/articles/subcategories?category=health').then(r => r.json()),
        ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      <HeroSection category="Health" articles={articles} />
...
function SubcategorySection({ subcategory }: { subcategory: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles?category=health&subcategory=${encodeURIComponent(subcategory)}&limit=6`)
      .then(r => r.json())
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map(a => (
          <div key={a.id} className="bg-white dark:bg-gray-900 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
            <a href={`/article/${encodeURIComponent(a.title)}`} className="block group">
